...
class ChatWindow(QMainWindow):
    def __init__(self):
        ...
        with open('default_setting.txt', encoding='utf-8') as f:
            self.default_setting = f.read()
        self.history = ''
    ...
    def send_message(self):
        ...
        if message:
            self.text_browser.append('<img src="Default.png", height=25>  User:')
            self.text_browser.append(message)
            # response = self.chatWithLLM(message)
            if 'gpt' in settings['Model']:
                self.text_browser.append('<img src="GPT.png", height=25>  LLM:')

    def chatWithLLM(self, message):
        if 'gpt' in settings['Model']:
            if self.history == '':
                completion = client.chat.completions.create(
                    model=settings['Model'],
                    messages=[
                        {'role': 'system', 'content': self.default_setting},
                ...
                completion = client.chat.completions.create(
                    model=settings['Model'],
                    messages=[
                        {'role': 'system', 'content': f'The history of the chat: {self.history}'},
                        {'role': 'system', 'content': message},
                    ]
                )
            response = completion.choices[0].message.content
            self.history += message + response
            return response
